"""Seed the sales-target and promotion codes, and reconcile the roles.

Every endpoint added since 2026-09-02 -- promotions, coupons, customer groups
and now sales targets -- calls ``require_permission`` on a code no existing
database has a row for. An unseeded code cannot be attached to any role, so
the endpoint quietly becomes platform-admin-only: their check short-circuits
the lookup. That is how twelve codes sat unusable until 2026-08-09, and it is
why every new code needs a migration as well as a seed entry.
...
revision: str = "20260903_0112"
down_revision: str | Sequence[str] | None = "20260903_0111"
