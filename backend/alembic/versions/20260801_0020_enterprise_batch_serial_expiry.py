...
def upgrade() -> None:
    ...
    op.add_column("products", sa.Column("track_lot", sa.Boolean(), nullable=False, server_default=sa.text("false")))
    op.add_column("products", sa.Column("track_serial", sa.Boolean(), nullable=False, server_default=sa.text("false")))
    op.add_column("products", sa.Column("track_expiry", sa.Boolean(), nullable=False, server_default=sa.text("false")))
    op.add_column("products", sa.Column("track_manufacturing_date", sa.Boolean(), nullable=False, server_default=sa.text("false")))
    op.add_column("products", sa.Column("track_warranty", sa.Boolean(), nullable=False, server_default=sa.text("false")))
    op.add_column("products", sa.Column("allow_negative_stock", sa.Boolean(), nullable=False, server_default=sa.text("false")))
    op.add_column("products", sa.Column("require_batch_on_receipt", sa.Boolean(), nullable=False, server_default=sa.text("false")))
    op.add_column("products", sa.Column("require_batch_on_issue", sa.Boolean(), nullable=False, server_default=sa.text("false")))
    op.add_column("products", sa.Column("require_serial_on_receipt", sa.Boolean(), nullable=False, server_default=sa.text("false")))
    op.add_column("products", sa.Column("require_serial_on_issue", sa.Boolean(), nullable=False, server_default=sa.text("false")))
    ...
    op.create_table(
        ...
        *_base_columns(),
        ...
        sa.UniqueConstraint("firm_id", "batch_number", "product_id", name="UQ_batches_firm_batch_product"),
    )
    ...
    op.create_table(
        ...
        *_base_columns(),
        ...
        sa.Column("lot_type", sa.String(length=50), nullable=False, server_default="PRODUCTION"),
        ...
        sa.UniqueConstraint("firm_id", "lot_number", "product_id", name="UQ_lots_firm_lot_product"),
    )
    ...
    op.create_table(
        ...
        *_base_columns(),
        ...
        sa.UniqueConstraint("firm_id", "serial_number", "product_id", name="UQ_serial_numbers_firm_serial_product"),
    )
    op.create_index("IX_serial_numbers_firm_id", "serial_numbers", ["firm_id"])
    op.create_index("IX_serial_numbers_firm_product", "serial_numbers", ["firm_id", "product_id"])
    op.create_index("IX_serial_numbers_firm_status", "serial_numbers", ["firm_id", "status"])

    # 5. Add FK columns to inventory_transactions
    op.add_column("inventory_transactions", sa.Column("batch_id", sa.Uuid(), nullable=True))
    op.add_column("inventory_transactions", sa.Column("lot_id", sa.Uuid(), nullable=True))
    op.add_column("inventory_transactions", sa.Column("serial_id", sa.Uuid(), nullable=True))
    op.create_foreign_key(
        "FK_inventory_transactions_batch_id", "inventory_transactions", "batches", ["batch_id"], ["id"], ondelete="RESTRICT"
    )
    op.create_foreign_key(
        "FK_inventory_transactions_lot_id", "inventory_transactions", "lots", ["lot_id"], ["id"], ondelete="RESTRICT"
    )
    op.create_foreign_key(
        "FK_inventory_transactions_serial_id", "inventory_transactions", "serial_numbers", ["serial_id"], ["id"], ondelete="RESTRICT"
    )
    ...
    op.create_index("IX_inventory_transactions_serial_id", "inventory_transactions", ["serial_id"])


def downgrade() -> None:
    op.drop_index("IX_inventory_transactions_serial_id", table_name="inventory_transactions")
    ...
    op.drop_constraint("FK_inventory_transactions_serial_id", "inventory_transactions", type_="foreignkey")
    op.drop_constraint("FK_inventory_transactions_lot_id", "inventory_transactions", type_="foreignkey")
