"""Seed ``CUSTOMER_MANAGE_SETTINGS`` and reconcile system role grants.

Credit policy decides whether a firm's trading stops, so it is not covered by
``CUSTOMER_UPDATE``: anyone who may edit a customer record would otherwise be
able to switch the block off. A code with no ``permissions`` row cannot be
attached to any role, which silently makes its endpoint platform-admin-only --
so a new code always needs this insert as well as the seed change.

The reconciliation is written against the seed constants rather than the one
new code, so it also repairs any other drift between the seed and a database
that has been upgraded past ``20260809_0044``.
"""
...
revision: str = "20260810_0058"
down_revision: str | Sequence[str] | None = "20260810_0057"
