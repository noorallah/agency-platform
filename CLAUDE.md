...
As of 2026-09-06 `pytest` is **green (1,239 unit + 48 integration)** and every test file also passes standalone — `tests/conftest.py` imports all model modules so `Base.metadata.create_all` sees the whole schema regardless of test order. Keep that list in step with `alembic/env.py`.
...
**The platform designation is its own claim, and a role code must never be able to spell it.** It used to be appended to the `roles` claim as the lowercase `"platform_admin"`, while genuine role codes are uppercase -- so a designation and a role code shared one list. `RoleCreate.code` requires `^[a-z0-9._-]+$`, which does not merely permit that spelling, it *requires* lowercase, and `FIRM_ADMIN` holds both `ROLE_CREATE` and `ROLE_ASSIGN`: create a role called `platform_admin`, assign it to yourself, sign in as a platform administrator. That reached every `require_platform_admin()` route, every permission through the short-circuit in `Principal.has_permission`, and **every firm's data**, because `optional_firm_scope` skips the membership check for one. Closed 2026-09-05; `create_role` also refuses the reserved codes case-insensitively, and `tests/unit/test_identity_hardening.py::test_a_role_cannot_spell_the_platform_designation` asserts both halves -- the code is refused, *and* a role row written by any other route still confers nothing.

**A platform administrator has two reaches, and there are three separate grants to narrow.** `platform_admins.scope` is `PLATFORM` or `ALL_FIRMS` (`20260905_0128`). `ALL_FIRMS` is the designation as it always behaved. `PLATFORM` **runs the platform** -- creates firms, their people, their settings, provisions storage -- and is refused a firm's books. Getting that right means narrowing all three grants, because any two without the third is not a narrowing at all: the membership exemption in `app/common/scope.py`, the short-circuit in `Principal.has_permission`, **and the stuffed `permissions` claim**, which `has_permission` reads directly -- a `PLATFORM` admin carrying operational codes would pass every firm permission check the moment they held a genuine membership. Each was perturbed separately and each failed the guard on its own. The refusal has no rule of its own: a `PLATFORM` admin is simply *not exempted*, so they meet the ordinary membership check, which means a designation is a **ceiling and not a floor** -- where they hold a real `UserFirm` row they act there as whatever their roles make them. `PLATFORM_OPERATOR_PERMISSION_CODES` is what they hold, deliberately **not** `PLATFORM_PERMISSION_CODES`, which answers a different question (what a firm admin may not *grant*) and is wrong in both directions: it omits `user`/`role`/`permission`, the operator's whole job, and includes `high_risk`, which is `VOID_INVOICE` and `EDIT_POSTED_TRANSACTION` on a firm's posted books. Existing rows were backfilled to `ALL_FIRMS` -- nobody was demoted -- but the column has **no server default** and the ORM default is the narrow one, so a row written without stating a reach gets the safe answer; state it explicitly where you create one, as `generate_sample_data.py` does. An **absent** claim on a token that carries the designation reads as `ALL_FIRMS`, which is the one place the powerful value is the default: it can only be a token minted before the scope existed, for somebody who by definition already had every firm, and demoting live administrators mid-session is a worse failure than an unchanged one for the few minutes an access token lives.

**A designation that exempts somebody from a check but leaves them unable to name a firm is not reach at all.** `GET /api/v1/me/firms` is what the desktop's firm switcher renders, and it returned `user_firms` rows only -- so an `ALL_FIRMS` administrator's reach depended on somebody having remembered to insert memberships. `superadmin` and `master.ops` were seeded into all four demo firms and worked; the bootstrap `platform-admin@agency.local` holds **zero**, so its switcher was **empty** while its token carried all 189 codes and the sidebar offered Sales, Purchases and Inventory -- every one of them refused on its first request, because `required_firm_scope` needs the header whatever the designation. It returns every active firm for `ALL_FIRMS` now, and deliberately **not** for `PLATFORM`, who are refused firm-owned routes and would get a switcher full of 403s. The membership still comes back where one exists, because it carries `is_primary`; a firm reached by the designation alone yields `None`, and the widened branch ranks `case((is_primary.is_(True), 0), else_=1)` rather than `is_primary DESC` -- most rows are NULL there, and the unit suite is SQLite, which sorts NULLs the opposite way from PostgreSQL in DESC. **The desktop's firm switcher is now the mode switch**: `requiresFirm` on `ModuleDefinition` *and* on `ModuleTabDefinition` (defaulting to true) hides what needs a firm when none is selected, declared per tab because Administration holds `users` and `roles` beside the tax, UOM and business-profile tabs that live in each firm's own store. A platform administrator **always starts on Platform**, whatever they were last in -- restoring a firm drops somebody with reach over every firm's books into one of them on a screen that looks like their own -- and `SessionController.resolveLandingFirm` is static and public so that rule is testable at all, the controller building its own `ApiClient`. Nobody else can reach the mode: for an ordinary user a null firm is an empty application rather than a mode, so `switchFirm(null)` refuses them.

**The screen that creates a firm was filed under a module that needs a firm.** `firms` was the first tab of **Masters** -- a firm's *own* master data -- so it was reachable only from inside another firm, and invisible to a platform administrator once platform mode existed. It is a tab of `administration` now with `requiresFirm: false`; `FIRM_VIEW` is a platform code no firm role may hold, so no firm user lost a tab. Two more things were in the way of actually setting a firm up. **`session.firms` is fetched once, at sign-in**, so a firm created minutes ago was in no switcher and `switchFirm` refused it as "not assigned to this user" -- true of the stale list and false of the platform; `SessionController.refreshFirms()` re-reads it and drops the current selection only if that firm has gone. And nothing joined creating a firm to configuring it, so **Open this firm** on the Firms grid refreshes and switches -- the business profile, financial year and chart of accounts live in the firm's own store and need `X-Firm-ID`. It is disabled for a dedicated firm whose storage is unprovisioned (switching in answers errors on every screen) and for a retired one. **And there were eight copies of the tab filter, not one**: `ModuleVisibility.tabIds` plus seven byte-identical copies in `desktop_shell.dart`, one per workspace. They drifted the moment `requiresFirm` was added -- the navigation tree hid the firm-owned tabs while every workspace's own tab strip went on offering them, so the sidebar and the screen it opened disagreed. `ModuleVisibility.tabsFor` is the only one now, and `module_visibility_test.dart` greps the shell for a ninth.

**`require_platform_admin()` on a route makes every permission code on it grant nothing**, because the designation comes from a `platform_admins` row and no role can reach it. `GET /api/v1/roles` had it: the only one of the four role endpoints on the designation -- `get_role`, `create_role` and `update_role` all take `ROLE_VIEW` -- and the only one of the three identity lists, beside `list_users` and `list_permissions`. So `ROLE_VIEW` was seeded, granted to `FIRM_ADMIN`, honoured everywhere except the one screen a firm administrator starts from, and the firm filtering already written inside `list_roles` was written for a caller who could never reach it. Found on 2026-09-05 by driving the endpoint with a real firm-admin token, not by reading the code. `tests/unit/test_platform_only_routes.py` pins the **64** routes that are deliberately platform-only (65 until `PUT /users/{id}/firms` came off the list on 2026-09-06 -- count them rather than trusting this), with the reason per group, and fails on the next one added by accident -- and on a pinned one that quietly stops being platform-only, which is the half that rots.

**Moving a claim is not done until nothing reads the old place, and a grep for the *new* name will not tell you.** #235 moved the platform designation out of the `roles` claim into its own; `audit_scope` in `app/common/audit/api/router.py` tested `"platform_admin" in principal.roles` in two places, and the sweep for `is_platform_admin` did not see them. The condition became permanently false, so **no platform administrator could read any audit trail** -- refused the platform trail by name, then refused every firm's trail by a membership check they hold no rows for. It survived a full green suite because `test_audit_trail_api.py` built its principal with `roles={"platform_admin"}`, the shape the application had stopped issuing: **a fixture that supplies the old shape cannot see the break**, which is why a suite going green after a claim moves proves less than it appears to. Fixed 2026-09-06, fixtures corrected -- with them in place the reverted router fails 2 of 6 -- and `test_no_module_reads_the_designation_out_of_the_roles_claim` greps `app/` for the old spelling, stripping comments first because a naive match flags the comment that warns against it, exactly as the `date.today()` sweep did.

**A firm administrator reads their own firm's audit trail as of 2026-09-06** (`20260906_0131`). `AUDIT_LOG_VIEW` is granted directly to `FIRM_ADMIN`, beside `SETTINGS_VIEW` and `SETTINGS_UPDATE` and for the same reason -- `PLATFORM_PERMISSION_CODES` says what a firm administrator may not *grant*, not what they may hold. Before it, Settings was offered on any of three codes while both its tabs demanded one of the other two, so the module opened empty. `audit_scope` reads one trail chosen by firm context and still applies the membership check, so `X-Firm-ID` gets that firm's history and nothing else: driven at 200 on their own firm and 403 on the platform trail. `DIAGNOSTICS_VIEW` stays out -- error reports are telemetry for whoever maintains the product, kept in one place rather than per firm -- so Diagnostics is still refused and its tab does not appear. **This moved a boundary somebody had drawn on purpose**: `test_firm_admin_has_no_platform_permissions_or_platform_access` listed `AUDIT_LOG_VIEW` beside `PLATFORM_VIEW`, `FIRM_CREATE` and `SYSTEM_CONFIGURATION`, and the full suite is what said so -- a targeted run over the files the change touched missed it, and the merge had already happened. The grant was kept, with the reasoning written into that test rather than the line quietly deleted: the other six reach the platform, and this one unlocks exactly one endpoint which is firm-scoped, so it confers a firm's own history and nothing else. **A permission's blast radius is the set of endpoints enforcing it, not the group it is filed under** -- `grep -rn CODE app/ --include=*.py` answers that in a second and is worth doing before adding any code to a role.

**A role can hold exactly the right codes and be offered no screen at all.** `CASHIER` holds `RECEIPT_CREATE`, `RECEIPT_VIEW`, `PAYMENT_CREATE` and `PAYMENT_VIEW`; Receipts and Payments are tabs of **Finance**, which was gated on `ACCOUNT_VIEW`, and a tab naming no codes **inherits the module's list** -- so a cashier signed in to an empty sidebar with a working account and correct permissions. The seeded `counter-sales` template pairs `CASHIER` with `BILLING_EXECUTIVE`, which is offered six modules, so a cashier on their own was not something anybody had made. Fixed 2026-09-06: Finance takes any of `ACCOUNT_VIEW`, `RECEIPT_VIEW`, `PAYMENT_VIEW` **and every tab names its own code**. Both halves are load-bearing and each was perturbed separately -- widening the module alone fixes the empty sidebar by handing a cashier the chart of accounts and the journal, which is the wrong fix twice over. Nobody lost a tab, which was checked rather than assumed: every code now on a tab is one that whoever held `ACCOUNT_VIEW` already had. Refunds keeps `ACCOUNT_VIEW` because no `REFUND_*` code exists and reversing a settlement is not the job of whoever took the money. `tests/unit/test_every_role_can_open_something.py` asks it of every seeded firm role, and asks the same question one level down -- a module offered with no openable tab opens empty, which `FIRM_ADMIN` still does on Settings, recorded in `_MODULES_THAT_OPEN_EMPTY` because fixing it means deciding whether a firm administrator may read their own firm's audit trail.

**`_operational_permissions` is a hand-kept list, and five groups drifted out of it.** `FIRM_ADMIN` and `FIRM_MANAGER` are built from that list in `app/identity/system_seed.py`, and a group added to `PERMISSION_GROUPS` is not added to it -- so `credit_note`, `proforma`, `einvoice`, `loyalty` and `tcs` each shipped with a module, a screen and a seeded gate while the role whose description is running the firm and every module could not open any of them. `SALES_MANAGER` names most of those codes individually, so the screens were reachable by somebody, which is why it survived until the access-control survey asked the question mechanically on 2026-09-06. Granted in `20260906_0130`, which names the twelve codes explicitly rather than reading `ROLE_PERMISSION_CODES` -- a migration is a record of one change on one date, and reading the live seed would make a replay next year do whatever the seed says then. It does **not** bump `authorization_version`: that would sign every firm administrator out mid-work to deliver a *widening* of their access, and the next token refresh carries it anyway. `tests/unit/test_firm_admin_holds_the_firms_own_modules.py` is the guard, and the point of it is the derived question rather than the five names -- it asks which groups are operational (neither in `PLATFORM_PERMISSION_CODES` nor firm administration) and fails when the list omits one, so it catches the sixth. Note what stays out: the `firm` group, because deciding which firms exist is not a firm's own business, and the two codes `FIRM_ADMIN` legitimately holds from inside `PLATFORM_PERMISSION_CODES` are `SETTINGS_VIEW` and `SETTINGS_UPDATE` -- that set answers "what may a firm admin not *grant*", which is a different question from what they may hold.

**A toolbar action that is not about the selected row must say so.** `ResourceAction.needsSelection` defaults to true, because almost every one is -- and the grid's toolbar gated *all* of them on a selection, so **Add existing user**, whose whole subject is somebody **not in the grid**, was dead until you clicked an unrelated person first. It read as a broken button. The action's `onInvoke` now takes a nullable item, and the four that do need a row keep the default and dereference it safely. Worth knowing when adding another: the guard for this was written with `context: null` first, which omits every dialog-opening action, so the loop ran over an empty list and **passed with the fix reverted** -- a real `BuildContext` from a `testWidgets` is what makes it bind, and perturbation is what caught it.

**A firm administrator can hire somebody who already has an account, and `GET /api/v1/users/lookup` is the only way they can see one.** `list_users` is scoped to the caller's own members and applies `search` *after* that filter, so an outsider could not be found -- `_get_user` even answers 404 rather than 403 so their existence is not disclosed. The lookup is a deliberate, narrow opening for the one job that needs it, and for a **firm caller** its limits are the design rather than an implementation detail: a **minimum of three characters** (`"a"` must not return the platform), a **cap of ten with no paging** (it answers "is this them?", not "who works here?" -- `page` is accepted and ignored, and anything past the first page is empty), a response of `{id, full_name, email, already_a_member}` and nothing else, **never which firms somebody belongs to**, platform administrators excluded as in `list_users`, and `USER_CREATE` rather than `USER_VIEW` -- reading your own firm's people and reaching across firms are different privileges. Enumeration by walking prefixes is still possible; that is accepted and written down rather than defended against, since `POST /api/v1/users` already answers 409 on a duplicate email and is a narrower oracle of the same kind. **A platform caller gets the directory on the same route as of 2026-09-08**, because the directory is theirs anyway: an empty `q` lists everybody with an account who is **not already in the firm on `X-Firm-ID`**, paged through the ordinary `page`/`page_size`, and a term of any length filters it. Members are *excluded* there rather than flagged -- the ask is "who can I add", and the firm's own people are in the grid beside the button -- so `already_a_member` is always false for a platform caller, as it always was. The desktop's Add existing user opens on that list for a platform administrator (`listsEveryone`) and still demands three characters from a firm administrator; the server enforces both, the flag only decides what the screen asks for. One route answering two callers differently is deliberate: two routes would be two implementations of "not a platform administrator" and two response shapes for one dialog. **`GET /api/v1/users/{id}/firms` returned every membership** on a route gated only by `ROLE_VIEW`, so any firm administrator holding a user id could read which firms that person belonged to, for every user rather than only shared ones; it takes the caller's reach now. And `UserResponse.belongs_to_other_firms` carries onto the row the fact that `_assert_exclusive_firm_user` acts on, so the grid disables Edit instead of offering a form that cannot save -- the refusal names the situation now, and says what the administrator *can* still do.

**A firm administrator staffs the firms they administer, and `FIRM_VIEW` is not one of their codes.** `FIRM_ADMIN` holds `USER_CREATE`, `USER_VIEW`, `USER_UPDATE`, `ROLE_ASSIGN` and `ROLE_VIEW` -- everything running a firm's people needs -- and **not** `FIRM_VIEW`, which is a platform code, one of `PLATFORM_PERMISSION_CODES` a firm administrator may not even grant. The desktop's New-user gate demanded all five, so the single role whose whole job is running a firm's people was refused the New and Edit buttons, holding all four codes that matter and failing on a fifth it can never be given. The firm picker was why it was listed, and that is fixed where the problem is: the form reads `/api/v1/me/firms` for anybody who is not a platform administrator, because `/api/v1/firms` is platform-only and came back empty for them. `userDefinition` is public for the same reason `permissionDefinition` is -- no test instantiates `DesktopShell`, so a private definition is one nothing can interrogate. **`PUT /api/v1/users/{id}/firms` came off the platform-only list on 2026-09-06**: its reason was "which firms a person belongs to is a cross-firm fact, so not a decision any single firm's administrator can make", which is true in its first half and a non-sequitur in its second -- an administrator of two firms putting a new hire in both is the ordinary case and was refused outright. The cross-firm part is **reach** instead: `_firms_the_caller_may_staff` limits them to firms where they hold `USER_CREATE` **in that firm** rather than firms they merely belong to, since somebody who administers one firm and sells in another must not staff the second. Two rules follow. A firm outside that reach is **refused by name**, never dropped -- a request that silently does less than it said is how somebody comes to believe a user was added. And the call **merges rather than replaces**: `set_user_firms` soft-deletes every membership the request did not name, which is right when the caller can see them all and destructive when they cannot, so memberships outside the reach are carried through untouched. A scoped caller also **does not move the primary firm** -- it is one flag across every firm a person belongs to, held by `UQ_user_firms_active_primary`, so setting it would either collide with a primary they cannot see or quietly demote it; only a person with none at all gets one, so a new hire still lands somewhere. `docs/USER_ADMINISTRATION_GUIDE.md` is the how-to.

**The user form writes one tier, and Roles by firm writes the other.** A platform administrator's Roles field is the **global** set (`user_roles.firm_id IS NULL`); a firm administrator's is their own firm's, scoped by `_firm_scope` on the server; and a firm-tier role is written on **Roles by firm** and nowhere else, whoever the caller is. Between 2026-09-07 and 2026-09-08 the form carried two more writers for a platform caller -- an **Apply roles to** picker at create and a second column, **Roles in the firm you are working in**, keyed off the firm switcher -- and both came out on 2026-09-08. Two places that write the same row are two places that can disagree, and a write keyed off the switcher is exactly the shape that had put roles in a tier nobody chose. Each caller still *sees* the tier they cannot write, read-only: **Also applies here** for a firm administrator, **Roles in specific firms** for a platform one, which now lists every firm rather than skipping the selected one. **Roles by firm was unusable by a firm administrator until the same day**: it read the firm names from the platform-only `/api/v1/firms`, so the one screen the form sent them to answered 403 and showed no firm at all, and `firm_roles_dialog_test.dart` could not see it because its fake answered that path for everybody -- it refuses it now, the way the server does. The dialog takes `firmsResource` (`'me/firms'` for a firm caller) and `staffableFirmIds`, `PermissionService.firmsHolding('USER_CREATE')`, which is the client-side spelling of `_firms_the_caller_may_staff`, so a firm administrator is offered exactly the sections a Save would be accepted for; no overlap says "no firm you administer" rather than "no firm yet", which would be false. And `GET /users/{id}/firms/{firm_id}/roles` is held to that reach too -- it answered for any firm to anybody holding `ROLE_VIEW` and a user id, the disclosure `GET /users/{id}/firms` had already been closed for.

**User-Firm Assignments is a platform administrator's tab, and a tab-level `requiresPlatformAdmin` is how that is said.** The screen is a strict subset of the Users form -- the same Firms and Primary firm fields, the same `PUT /users/{id}/firms` -- so for a firm administrator it was a third door onto one room beside Users → Edit → Firms and Add existing user; for a platform administrator attaching people to firms across the platform it is the lighter tool, and stays. The flag exists on `ModuleTabDefinition` as of 2026-09-08, the twin of the one `ModuleDefinition` already had, and `ModuleVisibility.tabsFor` reads it **before** the permission list. It is a flag and not `FIRM_VIEW` for the reason the module-level one is: a platform administrator passes permission checks by designation rather than by holding codes, so a code list is the wrong instrument -- `FIRM_VIEW` on this tab once hid it from the one role whose job it then was while the definition's own `canUseAction` said otherwise (#240, #249). The tab keeps `USER_VIEW`/`USER_UPDATE` beside the flag so the toolbar inside stays gated on something if the flag is ever removed. `tests/unit/test_every_role_can_open_something.py` reads the tab-level flag the way it reads the module-level one, so a flagged tab is deliberately closed to firm roles rather than an accident to record in `_TABS_NO_FIRM_ROLE_CAN_OPEN`. This is a UI-affordance decision, not an authorization one: the route stays `USER_UPDATE` with reach.

**A firm's audit trail is its own store *plus* the platform rows that belong to it.** `users`, `roles` and `user_firms` live only in the platform schema, so administering a firm's people runs on the platform session and `record_audit` writes there -- carrying the firm on `firm_id`, in a store the firm cannot read. A firm administrator could not see their own hiring, role edits or promotions. `AuditLogReader.list_events_with` merges the two on the **read**, and the write is deliberately left where it is: a DATABASE-mode firm is a separate database, possibly on a separate server, so writing the audit row there would be a second transaction -- the promotion could commit and its record fail, or the reverse. Today they commit together, which is worth more than the row's physical location. Three things to know. The merge is **exact, not a per-store pager**: `page * page_size` is taken from each store and the slice comes out of the combined order, because paging each separately repeats and drops rows at every boundary -- so deep paging costs more here, which is acceptable while `MAX_PAGE_SIZE` is 100. Both stores get the **same filters**, or the answer is a half-truth. And **a store is never merged with itself**: the unit suite builds one SQLite schema holding every table, so both dependencies resolve to the same session there, and merging would double every row and the total -- history nobody wrote. `tests/integration/test_audit_trail_spans_two_stores.py` is where this is actually checked, because a single schema cannot express it.

**An integration fixture that creates schemas needs its own engine.** `tests/integration/conftest.py`'s `audit_store_pair` builds one with `create_engine` and disposes it. Sharing the session-scoped `engine` returns its connections to a pool other tests draw from, and several of those set `search_path` on the connection and depend on it -- so a schema created and dropped in one file surfaced as `relation "firms" does not exist` in `test_multi_schema_tenancy.py`, a file it does not touch. Two more traps in the same corner, both of which cost a run each: a session left open sits `idle in transaction` holding a lock, and the fixture's `DROP SCHEMA ... CASCADE` then blocks on it **for ever** -- the test hangs at teardown rather than failing, which reads as a slow database and is not one (`pg_stat_activity` showing `Lock/relation` is what says so). And dropping several 182-table schemas in one transaction exhausts PostgreSQL's lock table -- "out of shared memory", whose hint names the max_locks_per_transaction setting -- so drop them one at a time under AUTOCOMMIT. A fixture that needs one table should build one table -- `audit_logs` carries no foreign keys, deliberately, since it spans stores.

**Applying a template is how a promotion is made, so the trail names the job.** `user_template.applied` recorded that a template was applied, to whom and by whom, and not **which** one -- so the history said somebody's access changed and nothing about what it changed to, on the one call whose whole purpose is changing it. It carries `template_id`, `template_code` and the granted `role_ids` now; the code travels beside the id because a template can be retired and an id alone then points at a row nobody can name. `clone_user` already recorded its `source_user_id` for the same reason, which is what made the omission visible. Note where the row lands: user administration is a platform path, so `record_audit` runs on the platform session and a firm administrator reading their own firm's Audit Logs does **not** see their own staffing decisions -- the row carries `firm_id`, but it is stored centrally and needs platform authority to read.

**A user is hired into a job in one step, and the create form has two boxes that could both decide the roles.** `Job template` on `Administration → Users → New` applies the job's roles as part of the save; `Roles` picks them by hand. Both are on screen, so one has to win -- **naming a job decides**, and the helper text under `Roles` says so rather than leaving it to be found in the audit trail. The field is `createOnly`: afterwards the person is an ordinary user and **Apply job template** on the grid is how a job is re-applied, so a field on the edit form would suggest the user stays tied to the template, which is exactly what a template is not. It goes through `apply_user_template` -- the same call the grid action makes -- so the firm-scope check that refuses a platform or cross-firm role applies unchanged and there is one implementation of it rather than two. `saveAssignments` on a `ResourceDefinition` runs after **create as well as update**, with the new id, which is what makes one step possible at all.

**Cloning a user copies access and nothing else, and the two halves are separate decisions.** `POST /api/v1/users/{id}/clone` is the other half of the template story and the more common one -- an administrator usually has a person in mind rather than a written-down job. The new user is **built from scratch** and given the source's roles and firm memberships; the mobile number, employee code, joining date, photo, password, login history, password history and audit trail are not copied, because they belong to the person rather than the job. That is the whole reason it is not "duplicate the row": a row copy carries all of it, silently. The clone always starts with `force_password_change` -- a password somebody else chose is not a password. **The platform designation is never copied**, and there are two locks on it: `_get_user` already excludes platform administrators from firm-scoped administration, so a firm administrator cannot reach one to clone, and a platform caller who can gets their roles and not the `platform_admins` row. `ROLE_ASSIGN` gates it, not `USER_CREATE`: it hands somebody a set of roles, so whoever may open accounts but not grant access must not be able to copy access instead. A firm caller copies only what their scope can see -- the firm-scoped rows plus the unscoped firm roles -- so another firm's roles stay invisible.

**A platform caller names the firm; a firm caller cannot.** `_target_firm` in `IdentityService` resolves it for both the template endpoints and the clone. A platform caller's scope resolves to null, which for a template means **offered to every firm** -- so a "Kitchen Staff" template written while setting up a restaurant was silently published to the wholesaler and the pharmacy too, and applying one granted the job globally rather than in the firm being set up. `firm_id` on the request body fixes both, and null keeps the old meaning so nothing that worked before changes. A firm caller naming a different firm is **refused rather than ignored**: silently writing it somewhere else is how the business-profile assignment endpoints came to report success while changing nothing. The roles in a template are validated against the firm that will *own* it rather than the caller's scope, or a platform operator could bundle a role that firm could never assign.

**A firm builds its own access without anybody writing code, and the chain is three links.** A permission is a capability, a **role** names a set of permissions, a **template** names a set of roles -- and a firm administrator may write their own at both the role and the template level, so nothing limits them to the twelve seeded roles or the eleven seeded templates. Driven end to end on 2026-09-06: create a firm-scoped role, put permissions on it, bundle it in a firm template, hire somebody into that template, and they hold exactly it. **167 of the 189 codes are assignable by a firm administrator** -- `list_permissions` filters `PLATFORM_PERMISSION_CODES` out of a firm-scoped read, so the other 22 are not merely refused on assignment but never offered, and `set_role_permissions` refuses them again if one is named directly. Two properties worth knowing before changing any of it: a role is **not versioned**, so editing one changes what everybody already holding it can do on their next token refresh; and `create_role` refuses `platform_admin` and the twelve seeded codes case-insensitively, which is the defence-in-depth half of the escalation fix. `docs/USER_ADMINISTRATION_GUIDE.md` sections 5 and 6 are the how-to.

**A user template is a named bundle of roles, and deliberately not a dormant user row.** `app/identity` owns `user_templates` / `user_template_roles` (`20260905_0129`), and a firm administrator hires by naming a job -- eleven are seeded platform-wide (`SYSTEM_USER_TEMPLATES`), a firm may add its own, and `firm_id IS NULL` means offered to everybody. Cloning a *user* was the alternative and it carries everything a user has -- an email, a password, memberships, an audit trail, a login history -- and the clone quietly inherits whatever was edited after the template was written. Four things to know. **Applying one is a plain `set_user_roles`**, so the firm-scope check that refuses a platform or cross-firm role applies unchanged and there is one implementation of it rather than two; what the user holds afterwards is an ordinary role set the administrator edits in the ordinary way, and **nothing on the user records which template they came from** -- a user who has since been edited is no longer described by it. **The bundle is validated at creation, not only at apply**, or a template naming an unassignable role fails on whoever tries to use it weeks later with nothing to say the template was wrong rather than their permissions; `PLATFORM_ADMIN` carries every seeded code, so a firm template able to name it would be a second door onto the same room. **The code is unique per scope**, which needs two partial indexes rather than one on `(firm_id, code)` -- PostgreSQL treats NULLs as distinct, so a single key would let the platform hold ten templates all called `counter-sales`. And **the eleven are inserted by the migration, not by `seed_system_rbac`**: that seeder is called only by `generate_sample_data.py` and never at startup, so a live database gets seeded rows from migrations. Written the other way first, and driving the endpoint answered **zero templates for every firm** -- which is how a feature ships unreachable.
