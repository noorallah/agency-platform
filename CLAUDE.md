...
**A firm administrator can hire somebody who already has an account, and `GET /api/v1/users/lookup` is the only way they can see one.** `list_users` is scoped to the caller's own members and applies `search` *after* that filter, so an outsider could not be found -- `_get_user` even answers 404 rather than 403 so their existence is not disclosed. The lookup is a deliberate, narrow opening for the one job that needs it, and its limits are the design rather than an implementation detail: a **minimum of three characters** (`"a"` must not return the platform), a **cap of ten with no paging** (it answers "is this them?", not "who works here?"), a response of `{id, full_name, email, already_a_member}` and nothing else, **never which firms somebody belongs to**, platform administrators excluded as in `list_users`, and `USER_CREATE` rather than `USER_VIEW` -- reading your own firm's people and reaching across firms are different privileges. Enumeration by walking prefixes is still possible; that is accepted and written down rather than defended against, since `POST /api/v1/users` already answers 409 on a duplicate email and is a narrower oracle of the same kind. **`GET /api/v1/users/{id}/firms` returned every membership** on a route gated only by `ROLE_VIEW`, so any firm administrator holding a user id could read which firms that person belonged to, for every user rather than only shared ones; it takes the caller's reach now. And `UserResponse.belongs_to_other_firms` carries onto the row the fact that `_assert_exclusive_firm_user` acts on, so the grid disables Edit instead of offering a form that cannot save -- the refusal names the situation now, and says what the administrator *can* still do.
