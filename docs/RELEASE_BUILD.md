...
| Server, fresh or upgrade: data folder | Before anything else, `Protect-DataRoot` removes inheritance on `C:\ProgramData\Agency Platform`: Administrators and SYSTEM full control, NetworkService modify on `pgdata` and `logs\database`, the server's virtual account modify on `logs` and `storage`, every user modify on `logs\client` and list-only on `logs`; nobody else reads the database files, attachments or dumps (D-QA-3). An upgrade applies the same to an existing install. Check with `icacls "C:\ProgramData\Agency Platform\pgdata"`: no `BUILTIN\Users` |
...
| Same version again (repair) | Before anything else, `InitializeSetup` says "Agency Platform <version> is already installed" and offers **Repair** or **Cancel** (a silent run repairs). Repair is the upgrade path unchanged -- backup, files, services, database, folder permissions -- worded as a repair on the preparing and finished pages (D-QA-2). Cancel changes nothing |
