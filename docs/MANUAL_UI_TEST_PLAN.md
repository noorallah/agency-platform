...
| 20.1b | **After 20.6**, select `plan20a` → **Edit**, then double-click the row, then the context menu's Edit | All three refuse, and **none of them silently**: the button is disabled, and the other two say "… also works in another firm, so their profile is managed by a platform administrator. Use Roles by firm to set what they do in yours." The disabled button came first; the silent no-op on double-click read as a broken screen rather than a rule, which is why the refusal names the way round. *(Split out of 20.1a on 2026-09-15 after the run: the second half needs a user in two firms and **no seeded user is one** — `master.ops` and `superadmin` are, but `_get_user` excludes platform administrators from firm-scoped administration, so they are not even in the grid. 20.6 is what creates the row, so this cannot be done at 20.1.)* |
