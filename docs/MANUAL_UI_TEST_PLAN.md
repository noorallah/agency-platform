...
| # | Case | Expected |
| --- | --- | --- |
| 11.1 | Sales → **Geography**. Select any row: the right-hand **Territory tree** panel. **Expand all**. Then the ⧉ icon beside the panel's title (tooltip "Open the tree in a larger window") opens the whole tree in a window, already expanded, with Expand all / Collapse all and each node's ⋮ actions (Filter list, Quick create child). | Chennai Region (REGION) → North Zone and South Zone (TERRITORY) → the three routes (ROUTE). The grid's Hierarchy column carries the full path for every row. |
