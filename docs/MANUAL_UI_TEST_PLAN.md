...
| 9.7 | The converted order carries the quoted 9.25% as an agreed (typed) rate, so a coupon cannot lower it; use a fresh order instead. Sales Orders → **New Order**: Customer `WHOLE01C01`, one line `DETER1K` qty **12**, Discount % blank, **Coupon** `WELCOME10`, **Create draft**. Select it → **Edit** to read the line. Then replace the coupon with `WELCOME10B`, **Save order**, Edit again. | Toast "Order drafted. Approve it to commit the stock and the credit." Under the line's blank Discount % box: "Last priced at **4.45**% by a promotion" -- the list rate plus 2.5 compounded on what is left (2 + 2.5 × 0.98). With `WELCOME10B` the helper reads "Last priced at **2**% by the price list": that code reaches nothing until it is presented on the offer it belongs to. *(On the order converted in 9.6 the box itself reads 9.25 and the coupon changes nothing -- the quoted rate carries over as the deal; BACKLOG §31.14.)* |
| 9.8 | Edit the same order, Coupon `NOSUCHCODE`, **Save order**. | Saves with "Order updated." and the line keeps its list rate. A typo in a field that gives money away must not refuse a sale; the helper under Coupon already says "Unrecognised codes are ignored". Put `WELCOME10` back and save. This order, not the converted one, is what 9.9 onward uses. |
