...
| 9.7 | Sales Orders → select that SO (status DRAFT) → **Edit**. In **Coupon** type `WELCOME10`, **Save order**. Edit again to read the line. | Toast "Order updated." Under the line's blank Discount % box: "Last priced at **4.45**% by a promotion" -- the list rate plus 2.5 compounded on what is left (for a 12-unit line at 2%: 2 + 2.5 × 0.98). Edit again, replace the coupon with `WELCOME10B`, Save: the rate is unchanged -- that code reaches nothing until it is presented on the offer it belongs to. |
