...
| 9.16 | Select the invoice → **Approve**. Then Masters → Customers → `WHOLE01C01`. | Status **APPROVED**. Finance → Journal Entries shows the invoice: Dr Accounts Receivable, Cr Sales, Cr Output Tax (CGST 9 + SGST 9). The customer's **Outstanding** is up by the invoice's grand total. |
...
| 9.18 | Finance → **Receipts** → **Record Receipt**. **Received from** `WHOLE01C01`, **Amount** half of the invoice's outstanding, **Method** Bank, then under **Apply to invoices** click **Oldest first**, **Record receipt**. | The dialog shows a TCS notice for the receipt. Toast "RC-... recorded and posted to the ledger."; the row's subtitle reads "Cleared SI-..." and its badge **Applied**. Customers: Outstanding down by the amount **less 0.1% TCS** (a 241.60 receipt moves it 241.36); the invoice keeps the rest outstanding. |
| 9.19 | **Record Receipt** again for `WHOLE01C01`: Amount = **the invoice's remaining Outstanding + 100**, apply exactly the invoice's Outstanding to it (or **Oldest first** and trim), Record receipt. | The dialog's running line reads "100 left on account" before saving; the saved row's badge reads **On account 100** and the invoice drops out of the outstanding list. Customers: Outstanding falls by the **whole** receipt less TCS, and **Advance is unchanged** -- the customer still owes on seeded invoices, so the 100 is money against the account, not an advance. Advance rises only when a receipt exceeds everything the customer owes; never a negative balance either way. |
