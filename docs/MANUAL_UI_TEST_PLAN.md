...
All as `whole01.admin` in WHOLE01, in order: each step feeds the next. Quotations, Sales Orders, Delivery Notes, Sales Invoices and Sales Returns are each their own sidebar entry (no groups); Credit Notes and Proforma are tabs of **Sales**; receipts are **Finance → Receipts**; the customer's balance is on **Masters → Customers** (the **Outstanding** and **Advance** columns). Seed facts the rates below rest on: `WHOLE01C01` Vijaya Super Stores has a 7.5% standing discount; the firm-wide `STANDING` price list on `DETER1K` has breaks 0 → 2%, 15 → 4.25%, 18 → 6.75%; `WHOLE01C02` Anand Agencies has its own `NEGOTIATED` list at a flat 9.25%; promotion `BULK5` gives 7.5% on a line of 25 or more (its earlier revision gave 5% and only reaches documents dated before the second seeded year); `WELCOME` gives 2.5% only when coupon `WELCOME10` is presented. DETER1K sells at 84 under `GST_18_LOCAL`. A resolved percentage is **not printed on a saved document**: read it by reopening the editor -- **Revise** on a quotation, **Edit** on a draft order -- where the Discount % box is refilled from what was stored (BACKLOG §31.14). Every screen reads once when opened: click **Refresh** after acting elsewhere.

| # | Case | Expected |
| --- | --- | --- |
| 9.1 | Quotations → **New Quotation**. Customer `WHOLE01C01`, leave "These prices stand until" as offered, **Add line**: Product `DETER1K`, Quantity **12**, leave **Discount %** empty (its helper reads "Blank takes this customer's 7.5%, or a price list where one applies."). **Create draft**. Select it → **Revise**. | Toast "QT-... drafted, good until ... Nothing is reserved by it." On Revise the line's Discount % box reads **2** -- the `STANDING` list's first break outranks the customer's 7.5% standing rate. Close the revision without saving. |
| 9.2 | Select the quotation → **Revise**, Quantity **18**, **Save revision**. Revise again to read the box. | **6.75** -- the ladder takes the highest break at or below 18, not the first one above zero. |
| 9.3 | Quotations → **New Quotation** for `WHOLE01C02`, one line `DETER1K` qty **18**, Discount % empty, Create draft, then Revise to read it. | **9.25** -- that shop's own `NEGOTIATED` list replaces the firm-wide ladder rather than amending it. |
| 9.4 | On the same quotation, Revise: Quantity **30**, Save revision, Revise to read. | **7.5** -- `BULK5` applies at 25 and above and a promotion outranks either price list, so it is 7.5 for either customer. *(The plan used to say "5 or 7.5"; 5 was the promotion's first revision and reaches only documents dated before the second seeded year.)* |
| 9.5 | Revise: type **0** into Discount %, Save revision, Revise to read. | The box reads **0** and the line total is the full 30 × 84. Zero is a refusal of every arrangement, not a silence. Then Revise once more, set Quantity back to **12** and clear the box, Save revision -- this is the quotation 9.6 converts. |
| 9.6 | Select the C02 quotation → **Mark as sent** → **Customer accepted** (dialog "Accept QT-...", give a Reason, **Accept**) → **Convert to order**. Then press **Convert to order** again. | Toasts "QT-... marked as sent...", "QT-... accepted. Converting it is what creates the order.", "QT-... became SO-.... The order reserves the stock when it is approved." The order is numbered in the `SO` series. The second Convert is refused with "Quotation QT-... already became SO-..." in the page's banner. |
| 9.7 | Sales Orders → select that SO (status DRAFT) → **Edit**. In **Coupon** type `WELCOME10`, **Save order**. Edit again to read the line. | Toast "Order updated." The line's Discount % box reads the list rate plus 2.5 compounded on what is left (for a 12-unit line at 2%: 2 + 2.5 × 0.98 = **4.45**). Edit again, replace the coupon with `WELCOME10B`, Save: the rate is unchanged -- that code reaches nothing until it is presented on the offer it belongs to. |
| 9.8 | Edit again, Coupon `NOSUCHCODE`, **Save order**. | Saves with "Order updated." and the line keeps its list rate. A typo in a field that gives money away must not refuse a sale; the helper under Coupon already says "Unrecognised codes are ignored". Put `WELCOME10` back and save. |
| 9.9 | Select the order → **Approve**. Then Masters → Products → DETER1K, or Inventory → Stock → Inventory for DETER1K in WHL_DC. | Status **APPROVED**; a credit warning toast may appear first (WHOLE01 warns at 80% and never blocks). Reserved on DETER1K in WHL_DC is up by 12. Sales → Promotions: the `WELCOME` offer's redemption for this order is **claimed** (it was pending while the order was a draft). |
...
| 9.12 | Delivery Notes → **New**, pick the order, on its line set **Delivering** to **5** (the field defaults to the 12 reserved), Warehouse `WHL_DC`, **Save Delivery Note**. Select the draft → **Approve** → **Dispatch**. | Toast "Delivery note DN-... created as a draft. Dispatching it is what moves the stock." Dispatch shows no toast; after Refresh the note reads **DISPATCHED**. Sales Orders: the order reads **PARTIALLY_DELIVERED**. Stock Ledger: `DISPATCH` −5 on DETER1K referenced by the DN. Finance → Journal Entries: the note's cost-of-goods entry. |
| 9.13 | Double-click the delivery note to open its view; note the line's **Unit Price**. Then open the sales order's view and compare. | **Identical.** The note ships the deal the order struck and does not re-read the customer's current rate. *(The order view prints the product as an id and the discount as an amount, not a percentage -- BACKLOG §31.3 and §31.14; the note view prints code and name.)* |
...
| 9.15 | Sales Invoices → **New Invoice**. In **Bill this delivery note** (helper "Only notes with something left to bill.") pick the first note (items read "DN-... · date · Anand Agencies"). Its line reads "dispatched 5 · at 84 less N%" with **Bill** defaulted to 5. Type **6** into Bill. | Refused before sending: "Only 5 left to bill." Set it back to **5**, **Create draft**: toast "Invoice created as a draft. Approve it to post the journal." A note that gave a unit free would show the free unit apart from the billable 5, and it could not be billed. |
| 9.16 | Select the invoice → **Approve**. Then Masters → Customers → `WHOLE01C02`. | Status **APPROVED**. Finance → Journal Entries shows the invoice: Dr Accounts Receivable, Cr Sales, Cr Output Tax (CGST 9 + SGST 9). The customer's **Outstanding** is up by the invoice's grand total. |
| 9.17 | With the invoice selected click the **Print settings** icon: set **How many copies** to 2 and give **Copy 1 label** / **Copy 2 label** (they prefill as ORIGINAL FOR RECIPIENT / DUPLICATE FOR TRANSPORTER), save. Then **Print**. | The PDF carries both parties' GSTINs, an HSN column (`340220`), the CGST/SGST split, the HSN-wise summary, "AMOUNT CHARGEABLE, IN WORDS", and the two labelled copies. *(Copies are not defaulted for an invoice until Print settings is saved, which is why the step comes first.)* |
| 9.18 | Finance → **Receipts** → **Record Receipt**. **Received from** `WHOLE01C02`, **Amount** half of the invoice's outstanding, **Method** Bank, then under **Apply to invoices** click **Oldest first**, **Record receipt**. | Toast "RC-... recorded and posted to the ledger."; the row's subtitle reads "Cleared SI-..." and its badge **Applied**. Customers: Outstanding down by the amount; the invoice keeps the rest outstanding. |
| 9.19 | **Record Receipt** again for `WHOLE01C02`: Amount **more than** what is still owed, **Oldest first**, Record receipt. | The dialog's running line reads "N left on account" before saving; the saved row's badge reads **On account N**. Customers: Outstanding is **0**, **Advance** is N -- never a negative balance. |
| 9.20 | Sales Invoices → New Invoice against the **second** delivery note (the 7), Create draft, Approve. Then Finance → Receipts: on the on-account receipt click the icon tooltipped **Apply to an invoice**. In "Apply RC-..." choose the new invoice, Amount up to the advance, **Apply**. | Toast "RC-... applied to SI-...". The dialog's own line says it: "Nothing moves in the ledger. The money arrived when the receipt was recorded." Finance → Journal Entries shows **no new entry**; Customers: Advance down by the amount applied, Outstanding down by the same. Applying more than is on account is refused: "RC-... has only N left unapplied." |
| 9.21 | On the first receipt (9.18) click the icon tooltipped **Reverse**, give a reason in "Why is it being reversed?", **Reverse**. | Toast "RC-... reversed."; the row's badge reads **Reversed**; Journal Entries shows the mirror entry `RC-...-REV`. Customers: Outstanding and Advance are back to exactly where they were before 9.18. Reversing it again is refused: "RC-... has already been reversed." |
| 9.22 | Sales Returns → **New Return**. **Returned against**: the first invoice (SI-... · date); **Line** 1; **Taken back into** `WHL_DC`; **Quantity returned** 2. **Create draft**. Select it → **Approve** → **Complete**. Try Quantity returned 9 on a new draft first if you want the cap: "Only 5 went out on this line." | Toasts "SR-... created as a draft...", "SR-... approved. Nothing has moved yet...", then "SR-... completed: 2 back on the shelf and <amount> credited to the customer." The detail pane's "What this moves" card ticks Stock, Customer and Ledger. Stock Ledger: `SALES_RETURN` +2; Customers: Outstanding down by the credit. |
...
| 9.24 | **Raise credit note** again on the same line with **Credit, before tax** larger than the line was charged (5 × 84 less its discount). | Refused with "A credit note cannot credit more than the line was charged: <charged> charged, <already> already credited." |
...
| 13.9c2 | Sign in as `elec01.admin`, Inventory → **Serials** | Up to twenty `MIX500-…` serials, AVAILABLE, each with a warranty year. Masters → Products → `MIX500` shows **Track serial** on. |
...
| 13.9e | Finance → Chart of Accounts → Edit `5000 Purchases` → tick **Requires a cost centre** → Save. Then Journal Entries → New, choose 5000 on a line | A **Cost centre \*** dropdown appears on that line and on no other; choose `SALES`. Save with it: posted. **(HTTP)** the same entry without `cost_center_id` on that line: **422**, the engine names the account. Untick the flag afterwards. |
