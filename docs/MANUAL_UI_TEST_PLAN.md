...
| 9.15 | Sales Invoices → **New Invoice**. In **Bill this delivery note** (helper "Only notes with something left to bill.") pick the first note (items read "DN-... · date · Anand Agencies"). Its line reads "dispatched 5 · at 84 less 2.5%" with **Bill** defaulted to 5. Type **6** into Bill. | Refused before sending: "Only 5 left to bill." (the server's own refusal, reached by an API client, is "Invoice quantity exceeds the available source quantity.") Set it back to **5**, **Create draft**: toast "Invoice created as a draft. Approve it to post the journal." A note that gave a unit free would show the free unit apart from the billable 5, and it could not be billed. |
...
| 9.18 | Finance → **Receipts** → **Record Receipt**. **Received from** `WHOLE01C02`, **Amount** half of the invoice's outstanding, **Method** Bank, then under **Apply to invoices** click **Oldest first**, **Record receipt**. | The dialog shows a TCS notice for the receipt. Toast "RC-... recorded and posted to the ledger."; the row's subtitle reads "Cleared SI-..." and its badge **Applied**. Customers: Outstanding down by the amount **less 0.1% TCS** (a 241.60 receipt moves it 241.36); the invoice keeps the rest outstanding. |
| 9.19 | **Record Receipt** again for `WHOLE01C02`: Amount = **the invoice's remaining Outstanding + 100**, apply exactly the invoice's Outstanding to it (or **Oldest first** and trim), Record receipt. | The dialog's running line reads "100 left on account" before saving; the saved row's badge reads **On account 100** and the invoice drops out of the outstanding list. Customers: Outstanding falls by the **whole** receipt less TCS, and **Advance is unchanged** -- the customer still owes on seeded invoices, so the 100 is money against the account, not an advance. Advance rises only when a receipt exceeds everything the customer owes; never a negative balance either way. |
