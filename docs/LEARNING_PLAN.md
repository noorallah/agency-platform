...
Four learning documents now exist and they do different jobs:

| Document | Use it when |
| --- | --- |
| [`FUNCTIONAL_GUIDE.md`](FUNCTIONAL_GUIDE.md) | You want to know what a module **does** -- workflow, settings, screens, tables -- without reading code |
...
**Two tracks, and they run in different orders.** The functional guide follows
the order a firm sets itself up in -- firm, profile, numbering, masters, then
buying and selling -- because you cannot raise an invoice before there is a tax
rate. This file follows dependency order, because you cannot read
`sales_invoice` before you have read `tax`. Take the functional track to learn
the product; take this one to change it.
...
[`FUNCTIONAL_GUIDE.md`](FUNCTIONAL_GUIDE.md)**: what it does, what to configure
first, its workflow, how to drive it from the desktop, and its tables. The endpoint
